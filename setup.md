...
We use a monorepo-style structure. You **must** install dependencies in the `/shared` directory first, as the other services depend on it.
...
## 5. Verification (The Health Check)
...
| :--- | :--- | :--- |
| **Backend Health** | [http://localhost:3000/health](http://localhost:3000/health) | `{"status":"active", "dbConnection":"connected", "userCount": 3}` |
...
* **JSON Mismatch:** If `userCount` is not `3`, verify that your database seed script ran successfully.
